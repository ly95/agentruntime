...
// validateOpenAIComparisonValue proves that raw is a single exact JSON value.
// It is the deep-compare hook used for values whose concrete Go type cannot be
// reflected (map[string]any, json.RawMessage); exactness is proven by decode.
func validateOpenAIComparisonValue(raw json.RawMessage, label string) error {
...
// validateOpenAIRawAgainstType proves that raw matches the concrete SDK type
// valueType. It delegates structural and closed-schema checks to
// validateOpenAIClosedSchema, then applies per-struct semantic rules.
func validateOpenAIRawAgainstType(raw json.RawMessage, label string, valueType reflect.Type) error {
	if err := validateOpenAIClosedSchema(raw, label, valueType); err != nil {
		return err
	}
	...
	if err != nil {
		return nil // non-object values are fully covered by the closed schema
	}
	return validateOpenAIStructSemanticFields(valueType, fields, label)
}

// validateOpenAIStructSemanticFields applies the per-type-name semantic rules
// that the structural closed schema deliberately leaves open: status domains,
// execution domains, and name-specific requirements.
func validateOpenAIStructSemanticFields(valueType reflect.Type, fields map[string]json.RawMessage, label string) error {
	for valueType.Kind() == reflect.Pointer {
		valueType = valueType.Elem()
	}
	if valueType.Kind() != reflect.Struct {
	...
	name := valueType.Name()
	if domain, ok := openAIStructStatusDomains[name]; ok {
		if err := openAIRequireDomain(fields, "status", domain, label); err != nil {
			return err
		}
	}
	switch name {
	case "ResponseInputItemToolSearchCall", "ResponseToolSearchOutputItemParamResp":
		if err := openAIRequireDomain(fields, "execution", []string{"server", "client"}, label); err != nil {
			return err
		}
	case "ResponseFunctionToolCall":
		if arguments, present := fields["arguments"]; present && openAINonNullRaw(arguments) {
			if err := validateOpenAIExactJSONString(arguments, label+".arguments"); err != nil {
				return err
			}
		}
	}
...
// validateOpenAIOptionalRawField validates the JSON kind of a field when it is
// present and (when nullable) non-null.
func validateOpenAIOptionalRawField(fields map[string]json.RawMessage, name, label, kind string, nullable bool) error {
...
// validateOpenAIKnownVariantFields rejects any present field that is not in the
// allowed set for the selected variant.
func validateOpenAIKnownVariantFields(fields map[string]json.RawMessage, label string, known, allowed []string) error {
...
func openAIRawBool(raw json.RawMessage, label string) (bool, error) {
	value, err := decodeExactJSON(raw)
	if err != nil {
		return false, fmt.Errorf("%w: OpenAI %s JSON is ambiguous or invalid: %v", ErrInvalidModelOutput, label, err)
	}
	result, ok := value.(bool)
	if !ok {
		return false, fmt.Errorf("%w: OpenAI %s must be a boolean", ErrInvalidModelOutput, label)
	}
	return result, nil
}
...
func validateOpenAIAnnotations(annotations []responses.ResponseOutputTextAnnotationUnion, label string) error {
...
func validateOpenAIOutputTextLogprobs(logprobs []responses.ResponseOutputTextLogprob, label string) error {
	for index, logprob := range logprobs {
		itemLabel := fmt.Sprintf("%s[%d]", label, index)
		if logprob.Token == "" {
			return fmt.Errorf("%w: OpenAI %s.token must not be empty", ErrInvalidModelOutput, itemLabel)
		}
		for topIndex, top := range logprob.TopLogprobs {
			if top.Token == "" {
				return fmt.Errorf("%w: OpenAI %s.top_logprobs[%d].token must not be empty", ErrInvalidModelOutput, itemLabel, topIndex)
			}
		}
	}
	return nil
...
func validateOpenAITextDeltaLogprobs(logprobs []responses.ResponseTextDeltaEventLogprob, label string) error {
	for index, logprob := range logprobs {
		itemLabel := fmt.Sprintf("%s[%d]", label, index)
		if logprob.Token == "" {
			return fmt.Errorf("%w: OpenAI %s.token must not be empty", ErrInvalidModelOutput, itemLabel)
...
func validateOpenAITextDoneLogprobs(logprobs []responses.ResponseTextDoneEventLogprob, label string) error {
	for index, logprob := range logprobs {
		itemLabel := fmt.Sprintf("%s[%d]", label, index)
		if logprob.Token == "" {
			return fmt.Errorf("%w: OpenAI %s.token must not be empty", ErrInvalidModelOutput, itemLabel)
